...
> data are open at the site on long-period ground motions of JMA
...
import numpy as np
import obspy
#import GetStationTable
#from obspy.signal.trigger import pk_baer

###
def GetCSVFileList(Dir):
	CSVFiles = glob.glob(Dir+'/*_acc.csv')
...
# get header values
	NSsampling, EWsampling, UDsampling, TriggerTime, StnCode, StnLon, StnLat = GetHeaderValues(iunit, CSVFile)
	print(TriggerTime)
...
##
def GetHeaderValues(iunit, CSVFile):
...
#
	DIR00 = '/Volumes/DATA01/JMA/strong_motion/2018/20180618_0758/acc/'

#	ifile = DIR00 + 'level.csv'
#	StnCodeDict = GetStationTable(ifile)
...
#	NSdigit, EWdigit, UDdigit, NSsampling, EWsampling, UDsampling, TriggerTime = ReadCSVFiles(DIR00, StnCode, hhmm)
		NSdigit, EWdigit, UDdigit, NSsampling, EWsampling, UDsampling, TriggerTime, StnCode, StnLon, StnLat = ReadCSVFile(CSVFile)

#		StnCode = StnCodeDict[address]
#		print(address, StnCode)
...
# create SAC files, adding appropriate header values
		...
		for icomp in range(3):
			...
			SACFileName = 'JMA' + StnCode + '.' + cyyyymmdd + '.' + chhmm + comp[icomp]
